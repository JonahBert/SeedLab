...
double desired_angle = 30;
float distancePerWheel = pi * wheelbase * desired_angle / 360.0;
float radiansPerWheel = 2 * distancePerWheel / diameter;
//Set parameters
float battery_voltage = 8.20;

double desired_pos[2] = {radiansPerWheel,-radiansPerWheel}; // desired motor position
...
void loop() {
  ...
  for(i=0;i<2;i++){
    ...
    error_between_angle = (pos_velo_rad_s[1] + pos_velo_rad_s[0])/2;

    // Adjust KP based on error, but ensure it stays within bounds
    Kp[1] = Kp[1] - error_between_angle * Kp[2];
    Kp[0] = Kp[0] + error_between_angle * Kp[2];
    ...
    if (desired_velocity[0] >= pi) desired_velocity[0] = pi; 
    if (desired_velocity[1] <= -pi) desired_velocity[1] = -pi;
