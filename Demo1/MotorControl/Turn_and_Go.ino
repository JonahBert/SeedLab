...
//Define necessary constand for pi, as well as necessary robot dimensions
float pi = 3.1416;
float diameter = 0.15;
float radius = diameter/2;
double wheelbase = 0.354; 
...
//set maximums for rotational and forward speed
double max_phi_dot = 1; 
double max_rho_dot = 1;

//define error and desired variables for our controllers
...
double desired_degrees_entered = 90;
double desired_feet_entered = 5;
double rho_desired = 0.3048 * desired_feet;
double phi_desired = desired_degrees * pi / 180;

bool time_done = 0; //signals when the robot has completed its turn, and is ready to move forward
...
//set variables for measuring speed and rotation speed
...
//Interrupt Service Requests for motor counting
...
void loop() {
  int i; //used in loop to measure speed on both motors
  //measured speed on both motors
  for(i=0;i<2;i++){
    ...
    //check if the time has passed for the turn to be complete
    if (current_time < 10) time_done = 0;
    else time_done = 1;

    //set the desired parameters for turning condition
    if (!time_done) {
      desired_degrees = desired_degrees_entered;
      phi_desired = desired_degrees * pi / 180;
      desired_feet = 0;
      rho_desired = 0.3048 * desired_feet;
    }

    //set the desired parameters for once the turn is complete
    if (time_done) {
      desired_feet = desired_feet_entered;
      rho_desired = 0.3048 * desired_feet;
    }
